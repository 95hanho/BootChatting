...
public class UserController {
	...
	// 첫 화면 및 로그아웃 뷰
	@GetMapping("/")
	public String loginMain(Model m, HttpSession session, @RequestParam(value = "ms", required = false) String ms,
	...
	// Get방식으로 다시 뷰로 가줘야 새로고침이나 뒤로가기 시 재호출가능
...
//		si.sessionTimeOutUpdate(session, status); // AOP로 처리하면 됨
	...
	public String loginInfoView(Model m, @RequestParam(value = "message", required = false) String message) {
	...
	public String userInsert(Model m, @ModelAttribute User user) {
		try {
			...
			return "redirect:logininfoView?message="+message;
		}
		return "redirect:loginComplete";
	}
